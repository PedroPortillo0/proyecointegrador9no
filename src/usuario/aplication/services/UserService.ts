...
export class UserService implements UserService {
    ...
    public async registerUser(name: string, email: string, password: string): Promise<User> {
        ...
        const user = new User(null, uuidv4(), name, email, hashedPassword);
    ...
    public async updateUser(uuid: string, updateData: { name?: string; email?: string; password?: string }): Promise<User> {
